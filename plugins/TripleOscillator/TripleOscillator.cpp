...
OscillatorObject::OscillatorObject( QObject * _parent, int _idx ) :
	Model( _parent ),
	m_volumeModel( MFact::create<float>(DefaultVolume / NUM_OF_OSCILLATORS, MinVolume,
			MaxVolume, 1.0f, this, tr( "Osc %1 volume" ).arg( _idx+1 )) ),
	m_panModel( MFact::create<float>(DefaultPanning, PanningLeft, PanningRight, 1.0f, this,
			tr( "Osc %1 panning" ).arg( _idx+1 )) ),
	m_coarseModel( MFact::create<float>(-_idx*KeysPerOctave,
			-2 * KeysPerOctave, 2 * KeysPerOctave, 1.0f, this,
			tr( "Osc %1 coarse detuning" ).arg( _idx+1 )) ),
	m_fineLeftModel( MFact::create<float>(0.0f, -100.0f, 100.0f, 1.0f, this,
			tr( "Osc %1 fine detuning left" ).arg( _idx+1 )) ),
	m_fineRightModel( MFact::create<float>(0.0f, -100.0f, 100.0f, 1.0f, this,
			tr( "Osc %1 fine detuning right" ).arg( _idx + 1 )) ),
	m_phaseOffsetModel( MFact::create<float>(0.0f, 0.0f, 360.0f, 1.0f, this,
			tr( "Osc %1 phase-offset" ).arg( _idx+1 )) ),
	m_stereoPhaseDetuningModel( MFact::create<float>(0.0f, 0.0f, 360.0f, 1.0f, this,
