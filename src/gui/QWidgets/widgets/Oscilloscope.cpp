...
Oscilloscope::Oscilloscope( QWidget * _p ) :
	...
	m_points( new QPointF[IEngine::Instance()->getAudioEngineInterface()->framesPerPeriod()] ),
	m_active( false ),
	...
	m_clippingColor(255, 64, 64)
{
	...
	setActive( IConfigManager::Instance()->value( "ui", "displaywaveform").toInt() );

	const fpp_t frames = IEngine::Instance()->getAudioEngineInterface()->framesPerPeriod();
	m_buffer = new sampleFrame[frames];

	clearInBufferManager( m_buffer, frames );
...
void Oscilloscope::setActive( bool _active )
{
	...
	if( m_active )
	{
		connect( getGUI()->mainWindow(),
					...
					this, SLOT(update()));
		connect( IEngine::Instance()->getAudioEngineInterface(),
	...
	else
	{
		disconnect( getGUI()->mainWindow(),
					SIGNAL(periodicUpdate()),
					this, SLOT(update()));
		disconnect( IEngine::Instance()->getAudioEngineInterface(),
			SIGNAL( nextAudioBuffer( const lmms::surroundSampleFrame* ) ),
			this, SLOT( updateAudioBuffer( const lmms::surroundSampleFrame* ) ) );
...
void Oscilloscope::paintEvent( QPaintEvent * )
{
	...
	if( m_active && !IEngine::Instance()->getSongInterface()->isExporting() )
	{
		IAudioEngine const * audioEngine = IEngine::Instance()->getAudioEngineInterface();

		float master_output = audioEngine->masterGain();

		const fpp_t frames = audioEngine->framesPerPeriod();
		IAudioEngine::StereoSample peakValues = audioEngine->getPeakValues(m_buffer, frames);
		...
		for( ch_cnt_t ch = 0; ch < DEFAULT_CHANNELS; ++ch )
		{
			...
			{
				sample_t const clippedSample = IAudioEngine::clip(m_buffer[frame][ch]);
