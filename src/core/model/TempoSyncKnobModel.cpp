...
// RTODO: make this global
class ISong {
public:
	virtual ~ISong() {}
	virtual void setModified(bool value = true) = 0;
	virtual bpm_t getTempo() = 0;
};

ISong* curSong = nullptr;
...
TempoSyncKnobModel::TempoSyncKnobModel( const float _val, const float _min,
	...
	m_custom( _parent )
{
	// connect( curSong, SIGNAL(tempoChanged(lmms::bpm_t)),
...
void TempoSyncKnobModel::setSyncMode( TempoSyncMode _new_mode )
{
	...
	calculateTempoSyncTime( curSong->getTempo() );
}
