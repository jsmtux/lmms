...
#include "interface/IControllerConnection.h"
...
AutomatableModel::AutomatableModel(
	...
	m_valueBuffer( static_cast<int>( framesPerPeriod() ) ),
...
class StaticAutomationInfo {
public:
	static const StaticAutomationInfo* Instance() {
		return &instance;
	}

	QString classNodeName() const {
		return "automationclip";
	}

	bool isAutomated(const AutomatableModel* model) const {
		return false;
	}
private:
	static StaticAutomationInfo instance;
};


bool AutomatableModel::isAutomated() const
{
	return StaticAutomationInfo::Instance()->isAutomated( this );
}
...
{
	// RTODO: find out what this does and fix
	// // compat code
	QDomNode node = element.namedItem( StaticAutomationInfo::Instance()->classNodeName() );
	// if( node.isElement() )
	// {
	...
	if( connectionNode.isElement() )
	{
		...
		if( thisConnection.isElement() )
		{
			setControllerConnection(IControllerConnection::createNew(nullptr));
...
void AutomatableModel::setControllerConnection( IControllerConnection* c )
{
...
// RTODO: Check wether this is needed
