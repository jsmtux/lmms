...
public:
	Note( const TimePos & length = TimePos( 0 ),
		...
		IDetuningHelper * detuning = nullptr );
	...
	virtual TimePos endPos() const
	{
		const int l = length();
		return pos() + l;
	}

	virtual const TimePos & length() const
	{
		return m_length;
	}

	virtual const TimePos & pos() const
	{
		return m_pos;
	}

	virtual TimePos pos( TimePos basePos ) const
	{
	...
	virtual volume_t getVolume() const
	{
		return m_volume;
	}
	...
	virtual panning_t getPanning() const
	{
		return m_panning;
	}
	...
	virtual IDetuningHelper * detuning() const
	{
		return m_detuning;
	}
