...
public:
	...
	AutomationNode(AutomationOwner* clip, float value, int pos);
	AutomationNode(AutomationOwner* clip, float inValue, float outValue, int pos);

	virtual ~AutomationNode() = default;
	 ...
	 * @param AutomationOwner* clip that m_clip will be
	 * set to
	 */
	inline void setClip(AutomationOwner* clip)
	{
		m_clip = clip;
	}
...
typedef QMap<int, AutomationNode> AutomationTimeMap;
using TimemapIterator = AutomationTimeMap::const_iterator;

class AutomationOwner
{
public:
	virtual const AutomationTimeMap & getTimeMap() const = 0;
	virtual AutomationTimeMap & getTimeMap() = 0;
	virtual void generateTangents() = 0;
	virtual void generateTangents(AutomationTimeMap::iterator it, int numToGenerate) = 0;
};
...
inline float INVAL(TimemapIterator it)
{
	return it->getInValue();
}

inline float OUTVAL(TimemapIterator it)
{
	return it->getOutValue();
}

inline float OFFSET(TimemapIterator it)
{
	return it->getValueOffset();
}

inline float INTAN(TimemapIterator it)
{
	return it->getInTangent();
}

inline float OUTTAN(TimemapIterator it)
{
	return it->getOutTangent();
}

inline int POS(TimemapIterator it)
{
	return it.key();
}
