 ...
 */
class LMMS_EXPORT PatternStore : public QObject, public IPatternStore, public TrackContainerCb
{
	Q_OBJECT
public:
	PatternStore();
	...
	{
		return m_patternComboBoxModel.value();
	}

	IComboBoxModelWrapper* patternComboboxModel() override {
		return &m_patternComboBoxModel;
	}

	virtual bool play(TimePos start, const fpp_t frames, const f_cnt_t frameBase, int clipNum = -1) override;
	...
	bar_t lengthOfPattern(int pattern) const override;
...
public slots:
	...
	void stop() override;
	void updateComboBox();
	void setCurrentPattern( const int val ) override
	{
		m_patternComboBoxModel.setValue( (float) val );
	}

private:
	TrackContainer m_trackContainer;

	ComboBoxModelWrapper m_patternComboBoxModel;
