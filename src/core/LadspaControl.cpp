	...
	m_tempoSyncKnobModel( 0, 0, 0, 1, m_port->max, this, m_port->name )
{
	if( m_link )
	{
		connect( &m_linkEnabledModel, &Model::dataChanged,
					 this, &LadspaControl::linkStateChanged,
					 Qt::DirectConnection );
	}
	...
	switch( m_port->data_type )
	{
		case TOGGLED:
			...
			connect( &m_toggledModel, &Model::dataChanged,
					 this, &LadspaControl::ledChanged);
			if( m_port->def == 1.0f )
			{
		...
		case ENUM:
			...
			connect( &m_knobModel, &Model::dataChanged,
						 this, &LadspaControl::knobChanged);
			// TODO: careful: we must prevent saved scales
		...
		case TIME:
			...
			connect( m_tempoSyncKnobModel.wrappedModel()->model(), &Model::dataChanged,
					 this, &LadspaControl::tempoKnobChanged);
			// TODO: careful: we must prevent saved scales
