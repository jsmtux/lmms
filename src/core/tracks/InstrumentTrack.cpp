...
IInstrumentTrack* createInstrumentTrack(ITrackContainer* tc) {
	return new InstrumentTrack( static_cast<TrackContainer*>(tc) );
}

InstrumentTrack::InstrumentTrack( TrackContainer* tc ) :
	TypedTrack( Track::InstrumentTrack, tc ),
	MidiEventProcessor(),
	...
	m_previewMode( false ),
	m_baseNoteModel(0, 0, NumKeys - 1, model(), tr("Base note")),
	m_firstKeyModel(0, 0, NumKeys - 1, model(), tr("First note")),
	m_lastKeyModel(0, 0, NumKeys - 1, model(), tr("Last note")),
	m_hasAutoMidiDev( false ),
	m_volumeModel( DefaultVolume, MinVolume, MaxVolume, 0.1f, model(), tr( "Volume" ) ),
	m_panningModel( DefaultPanning, PanningLeft, PanningRight, 0.1f, model(), tr( "Panning" ) ),
	m_audioPort( tr( "unnamed_track" ), true, &m_volumeModel, &m_panningModel, &m_mutedModel ),
	m_pitchModel( 0, MinPitchDefault, MaxPitchDefault, 1, model(), tr( "Pitch" ) ),
	m_pitchRangeModel( 1, 1, 60, model(), tr( "Pitch range" ) ),
	m_mixerChannelModel( 0, 0, 0, model(), tr( "Mixer channel" ) ),
	m_useMasterPitchModel( true, model(), tr( "Master pitch") ),
	m_instrument( nullptr ),
	m_soundShaping( this ),
	m_arpeggio( this->model() ),
	m_noteStacking( this->model() ),
	m_piano(this),
	m_microtuner()
{
...
IMidiClip* InstrumentTrack::createMidiClip(TimePos pos) {
	return dynamic_cast<IMidiClip*>(createClip(pos));
}
...
void InstrumentTrack::processAudioBuffer( sampleFrame* buf, const fpp_t frames, INotePlayHandle* n )
{
	...
	if( m_instrument->flags().testFlag( IInstrument::IsSingleStreamed ) &&
					...
					for (NotePlayHandle* nph : m_sustainedNotes)
					{
						if (nph && nph->isReleased())
						{
								...
								nph->OriginMidiInput)
							{
								...
								m_instrumentTrackModel.midiNoteOff( *nph );
							}
						}
					}
...
void InstrumentTrack::processOutEvent( const MidiEvent& event, const TimePos& time, f_cnt_t offset )
{
	...
	switch( event.type() )
	{
		case MidiNoteOn:
			...
			emit m_instrumentTrackModel.newNote();
			break;
		...
		case MidiNoteOff:
			...
			m_midiNotesMutex.unlock();
			emit m_instrumentTrackModel.endNote();
			break;
...
void InstrumentTrack::silenceAllNotes( bool removeIPH )
{
	...
	std::set types{PlayHandleType::TypeNotePlayHandle, PlayHandleType::TypePresetPreviewHandle};
	if( removeIPH )
	{
		types.insert(PlayHandleType::TypeInstrumentPlayHandle);
	}
	Engine::audioEngine()->removePlayHandlesOfTypes( this, types );
	Engine::audioEngine()->doneChangeInModel();
}
...
void InstrumentTrack::setName( const QString & _new_name )
{
	...
	emit m_instrumentTrackModel.nameChanged();
}
...
void InstrumentTrack::removeMidiPortNode( IDataFile & _dataFile )
{
...
IClip* InstrumentTrack::createClip(const TimePos & pos)
{
...
void InstrumentTrack::saveTrackSpecificSettings( QDomDocument& doc, QDomElement & thisElement )
{
	...
	if( m_instrument != nullptr )
	{
		...
		i.setAttribute( "name", m_instrument->name() );
		QDomElement ins = m_instrument->saveStateAndKey( doc, i );
...
void InstrumentTrack::loadTrackSpecificSettings( const QDomElement & thisElement )
{
	...
	bool reuseInstrument = m_previewMode && m_instrument && m_instrument->serializingObject()->nodeName() == getSavedInstrumentName(thisElement);
		...
		if( node.isElement() )
		{
			...
			else if(node.nodeName() == "instrument")
			{
				...
				else
				{
					delete m_instrument;
					m_instrument = nullptr;
					m_instrument = InstantiateInstrument(
						node.toElement().attribute("name"), this, &key);
					m_instrument->restoreState(node.firstChildElement());
					emit m_instrumentTrackModel.instrumentChanged();
				}
			}
					...
					!node.toElement().hasAttribute( "id" ))
			{
				...
				if (m_instrument->serializingObject()->nodeName() == node.nodeName())
				{
...
void InstrumentTrack::replaceInstrument(IDataFile& dataFile)
{
...
IInstrument* InstrumentTrack::loadInstrument(const QString & _plugin_name,
	const PluginDescriptor::Key *key, bool keyFromDnd)
{
	if(keyFromDnd)
		Q_ASSERT(!key);
	...
	lock();
	delete m_instrument;
	m_instrument = InstantiateInstrument(_plugin_name, this,
					key, keyFromDnd);
	unlock();
