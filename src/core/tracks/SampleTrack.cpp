...
ISampleTrack* createSampleTrack(ITrackContainer* tc) {
	return new SampleTrack(static_cast<TrackContainer*>(tc));
}
	...
	m_isPlaying(false)
{
	...
	connect(&m_mixerChannelModel, &Model::dataChanged, this, &SampleTrack::updateMixerChannel);
}
...
{
	Engine::audioEngine()->removePlayHandlesOfTypes( this, std::set{PlayHandleType::TypeSamplePlayHandle} );
}
...
IClip * SampleTrack::createClip(const TimePos & pos)
{
...
void SampleTrack::createSampleClip(const TimePos & pos, const QString & sampleFile ) {
	static_cast<SampleClip*>(createClip(pos))->setSampleFile(sampleFile);
}
