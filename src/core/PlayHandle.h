...
class LMMS_EXPORT PlayHandle : public ThreadableJob, public virtual IPlayHandle
{
public:
	enum
	{
	...
	PlayHandle( const PlayHandleType type, f_cnt_t offset = 0 );
	...
	bool usesBuffer() const override
	{
		return m_usesBuffer;
	}
	...
	sampleFrame * buffer() override;

private:
	PlayHandleType m_type;
	f_cnt_t m_offset;
	QThread* m_affinity;
