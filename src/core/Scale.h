...
class Interval : public IInterval, public SerializingObject
{
public:
	...
	float getRatio() const override {return m_ratio;}
...
class Scale : public QObject, public IScale, public SerializingObject
{
	Q_OBJECT
public:
	Scale();
	Scale(QString description, std::vector<IInterval*> intervals);

	QString getDescription() const override;
	void setDescription(QString description);

	const std::vector<std::unique_ptr<IInterval>> &getIntervals() const {return m_intervals;}
	const std::vector<QString> getIntervalStrings() const override {
		std::vector<QString> ret;
		for (auto& interval: getIntervals()) {
			ret.push_back(static_cast<Interval*>(interval.get())->getString());
		}
		return ret;
	}
	void setIntervals(std::vector<std::unique_ptr<IInterval>> input) {m_intervals = std::move(input);}
...
private:
	...
	std::vector<std::unique_ptr<IInterval>> m_intervals;      //!< a series of ratios that define the scale
