...
class Keymap : public QObject, public IKeymap, public SerializingObject
{
	Q_OBJECT
public:
	Keymap();
	...
	int getMiddleKey() const override {return m_middleKey;}
	int getFirstKey() const override {return m_firstKey;}
	int getLastKey() const override {return m_lastKey;}
	int getBaseKey() const override {return m_baseKey;}
	float getBaseFreq() const override {return m_baseFreq;}

	std::size_t getSize() const {return m_map.size();}
	int getDegree(int key) const override;
	int getOctave(int key) const;
	const std::vector<int> &getMap() const override {return m_map;}
