...
void ProjectJournal::undo()
{
	...
	{
		CheckPoint& c = m_undoCheckPoints.top();
		JournallingObject *jo = m_joIDs[c.joID];

		if( jo )
		{
			auto curState = createJournalDataFile();
			jo->saveState( *curState, curState->content() );
			m_redoCheckPoints.push( CheckPoint( c.joID, std::move(curState) ) );
...
void ProjectJournal::redo()
{
	while( !m_redoCheckPoints.isEmpty() )
	{
		...
		if( jo )
		{
			auto curState = createJournalDataFile();
			jo->saveState( *curState, curState->content() );
			m_undoCheckPoints.push( CheckPoint( c.joID, std::move(curState) ) );
...
void ProjectJournal::addJournalCheckPoint( JournallingObject *jo )
{
	if( isJournalling() )
	{
		...
		auto dataFile = createJournalDataFile();
		jo->saveState( *dataFile, dataFile->content() );

		m_undoCheckPoints.push( CheckPoint( jo->id(), std::move(dataFile) ) );
