...
// RTODO: implement this
std::unique_ptr<IDataFile> createJournalDataFile() {
	return std::unique_ptr<IDataFile>(nullptr);
}
...
private:
	...
	struct CheckPoint
	{
		...
		CheckPoint( jo_id_t initID = 0, const std::shared_ptr<IDataFile>&& initData = createJournalDataFile() ) :
			joID( initID ),
			data( initData )
		{
		}
		jo_id_t joID;
		std::shared_ptr<IDataFile> data;
	} ;
