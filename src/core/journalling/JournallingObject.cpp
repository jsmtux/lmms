...
#include "interface/IProjectJournal.h"

namespace lmms
{

IProjectJournal* projectJournal = nullptr;

JournallingObject::JournallingObject() :
	SerializingObject(),
	m_id( projectJournal->allocID( this ) ),
	m_journalling( true ),
...
void JournallingObject::addJournalCheckPoint()
{
	if( isJournalling() )
	{
		projectJournal->addJournalCheckPoint( this );
	}
}
...
void JournallingObject::changeID( jo_id_t _id )
{
	if( id() != _id )
	{
		JournallingObject * jo = projectJournal->
		...
		projectJournal->freeID( m_id );
		projectJournal->reallocID( _id, this );
		m_id = _id;
	}
}
