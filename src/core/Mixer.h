...
class MixerChannel : public IMixerChannel, public ThreadableJob
{
	public:
		...
		IBoolAutomatableModel* muteModel() override {
			return &m_muteModel;
		}
		...
		IEffectChain* fxChain() override {
			return &m_fxChain;
		}
		float peakLeft() override {
			return m_peakLeft;
		}
		float peakRight() override {
			return m_peakRight;
		}
		void setPeakLeft(float value) override {
			m_peakLeft = value;
		}
		void setPeakRight(float value) override {
			m_peakRight = value;
		}
		...
		void setColor (QColor newColor) override
		{
			m_color = newColor;
			m_hasColor = true;
		}
...
class LMMS_EXPORT Mixer : public IMixer
{
public:
	Mixer();
	~Mixer() override;
	...
	IMixerChannel* getMixerChannelInterface(std::size_t channel) override
	{
		return m_mixerChannels[channel];
	}
	...
	MixerRoute * createChannelSend(mix_ch_t fromChannel, mix_ch_t toChannel,
						   float amount = 1.0f) override;
	MixerRoute * createRoute( MixerChannel * from, MixerChannel * to, float amount );

	// delete the connection made by createChannelSend
	void deleteChannelSend(mix_ch_t fromChannel, mix_ch_t toChannel) override;
	...
	bool isInfiniteLoop(mix_ch_t fromChannel, mix_ch_t toChannel) override;
	...
	IFloatAutomatableModel * channelSendModel(mix_ch_t fromChannel, mix_ch_t toChannel) override;
	...
	void deleteChannel(int index) override;

	// delete all the mixer channels except master and remove all effects
	void clear() override;

	// re-arrange channels
	void moveChannelLeft(int index) override;
	void moveChannelRight(int index) override;

	// reset a channel's name, fx, sends, etc
	void clearChannel(mix_ch_t channelIndex) override;
	...
	bool isChannelInUse(int index) override;

	void toggledSolo() override;
	void activateSolo();
	void deactivateSolo();

	inline mix_ch_t numChannels() const override
	{
