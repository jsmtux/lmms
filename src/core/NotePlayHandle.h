...
class LMMS_EXPORT NotePlayHandle : public INotePlayHandle, public PlayHandle, public Note
{
	MM_OPERATORS
public:
	void * m_pluginData;
	...
	fpp_t fadeInLength() override {
		return m_fadeInLength;
	}
	void setFadeInLength(fpp_t len) override {
		m_fadeInLength = len;
	}
	...
	f_cnt_t framesLeft() const override;

	/*! Returns how many frames have to be rendered in current period */
	fpp_t framesLeftForCurrentPeriod() const override;
	...
	f_cnt_t totalFramesPlayed() const override
	{
	...
	/*! Returns list of note-play-handles belonging to given instrument track.
	    ...
	    are returned */
	static ConstNotePlayHandleList nphsOfInstrumentTrack( const ITrack* Track, bool allPlayHandles = false );
...
private:
	class BaseDetuning
	{
		MM_OPERATORS
	public:
		BaseDetuning( IDetuningHelper* detuning );
...
public:
	...
	static void release( INotePlayHandle * nph );
