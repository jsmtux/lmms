...
class LMMS_EXPORT ExportFilter : public IExportFilter, public Plugin
{
public:
	ExportFilter( const PluginDescriptor * _descriptor ) : Plugin( _descriptor, nullptr ) {}
	~ExportFilter() override = default;


	virtual bool tryExport(const TrackList &tracks,
				const TrackList &patternTracks,
				int tempo, int masterPitch, const QString &filename ) override = 0;
protected:
