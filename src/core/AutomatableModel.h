...
public:
	...
	*/
	...
	void setNodeName(QString _nodeName) {
	...
	IControllerConnection* controllerConnection() const
	{
		return m_controllerConnection.get();
	}

	void setControllerConnection( std::unique_ptr<IControllerConnection>&& connection );
...
private:
	virtual IAutomatableModelBase* thisInterface() = 0;
...
protected:
	...
	std::unique_ptr<IControllerConnection> m_controllerConnection;
...
public:
	...
	T value( int frameOffset = 0 ) const override
	{
		return AutomatableModel::value<T>( frameOffset );
	}

	// Interface hides that this inherits from model
	// Would be nice for it to be actually the case
	...
	{
		return AutomatableModel::setValue( value );
	}

	T initValue() const override
	{
		return AutomatableModel::initValue<T>();
	}

	void setInitValue( const T value ) override {
		AutomatableModel::setInitValue(static_cast<float>(value));
	}

	void incValue( int steps ) override
	{
		setValue( value() + steps * step() );
	}

	T step() const override {
	...
	{
		return AutomatableModel::range();
	}

	bool isAutomated() const override {
		return IAutomationClip::isAutomated( this );
	}

	void setUseControllerValue(bool b = true) override {
		return AutomatableModel::setUseControllerValue(b);
	}

	void setAutomatedValue( const float value ) override {
		return AutomatableModel::setAutomatedValue(value);
	}

	bool useControllerValue() override {
		return AutomatableModel::useControllerValue();
	}

	float stepAsFloat() const override {
		return AutomatableModel::step<float>();
	}
    ...
    jo_id_t id() const override {
		return JournallingObject::id();
	}
	...
	void setJournalling( const bool _sr ) override
	{
		AutomatableModel::setJournalling(_sr);
	}

    bool hasLinkedModels() const override {
		return AutomatableModel::hasLinkedModels();
	}

	void unlinkAllModels() override {
		AutomatableModel::unlinkAllModels();
	}

	IControllerConnection* controllerConnection() const override
	{
		return AutomatableModel::controllerConnection();
	}

	void setControllerConnection( std::unique_ptr<IControllerConnection>&& connection ) override {
		AutomatableModel::setControllerConnection(std::move(connection));
	}

    void setRange( const T min, const T max, const T step = 1 ) override {
		return AutomatableModel::setRange(min, max, step);
	}

	float scaledValue( float value ) const override {
		return AutomatableModel::scaledValue(value);
	}

	float inverseScaledValue( float value ) const override {
		return AutomatableModel::inverseScaledValue( value );
	}

	void addJournalCheckPoint() override {
		JournallingObject::addJournalCheckPoint();
	}
	void saveJournallingState( const bool newState ) override {
		JournallingObject::saveJournallingState(newState);
	}
	void restoreJournallingState() override {
		JournallingObject::restoreJournallingState();
	}

	float centerValue() const override {
		return AutomatableModel::centerValue();
	}

    bool isScaleLogarithmic() const override {
		return AutomatableModel::isScaleLogarithmic();
	}
    void setScaleLogarithmic( bool setToTrue = true ) override {
		AutomatableModel::setScaleLogarithmic(setToTrue);
	}
private:
	IAutomatableModelBase* thisInterface() override { return this; }
};
...
using AutomatedValueMap = QMap<IAutomatableModelBase*, float>;
